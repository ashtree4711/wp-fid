...
use App\Http\Operations\Holdings;
...
class UserController extends BaseController
{
   public function showProfile(ServerRequest $request){
       ...
       $user["leaflets_full"]=[];
       $user["holdings"]=Holdings::get($request);
       if (isset($_SESSION["leaflets"])){
           for ($i=0; $i<count($_SESSION["leaflets"]); $i++) {
               array_push($user["leaflets_full"], KugCatalog::getResultByUrl($_SESSION["leaflets"][$i],$request->getServerParams()["KUG_FID"]));
           }
       }
